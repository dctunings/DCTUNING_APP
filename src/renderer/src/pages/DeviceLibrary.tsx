...
const DEVICES: Device[] = [
  ...
  },
  {
    ...
    software: ['ECUFlash (open source)', 'RomRaider (open source)', 'EcuTek ProECU', 'Cobb Accessport (development)', 'Any J2534 app'],
    ...
    appCompatNote: 'Registers in Windows J2534 registry — appears in DCTuning J2534 PassThru device scanner. App can use it for OBD2 diagnostics. Flash ops require compatible J2534 flash software.',
  ...
  },
  {
    ...
    software: ['PCMTuner Software (J2534 flash suite, included)'],
    notes: 'PCMTuner ships with its own J2534 DLL that registers in Windows — meaning it works as a J2534 adapter with any compatible J2534 flash software. OBD2 only — no bench or boot capability. Popular clone-friendly dongle widely used in the tuning community.',
    appCompat: 'direct-j2534',
    appCompatNote: 'PCMTuner registers its J2534 DLL in Windows — auto-detected in DCTuning J2534 PassThru scanner. App uses it for OBD2 diagnostics. For ECU flash read/write, use the included J2534 flash software, then import the .bin into DCTuning Tune Manager.',
    recommended: ['Toyota/Subaru OBD flash', 'GM/Opel OBD flash', 'All-in-one J2534 bundle'],
  },
  {
    id: 'kt200',
    name: 'KT200',
  ...
  },
  {
    ...
    software: ['Any J2534-compliant flash software', 'WinFlash', 'DiagSoft'],
    notes: 'Full J2534-1 compliant pass-thru adapter — identical ECU flash coverage to a Tactrix Openport 2.0 at a lower price. Reads and writes ECUs on Toyota, Subaru, Honda, GM/Opel, Nissan, Mitsubishi via any J2534 flash software over OBD2. No bench/boot capability — OBD-only. Supports CAN (ISO 15765), K-Line (ISO 9141/14230), J1850 PWM/VPW. USB and Bluetooth versions available. Widely trusted in the Eastern European tuning community. Register its DLL in Windows once and it works with any J2534 software.',
    appCompat: 'direct-j2534',
    appCompatNote: 'Registers its J2534 DLL in Windows — appears in DCTuning J2534 PassThru scanner automatically. App uses it for OBD2 diagnostics (DTC, live PIDs, voltage). For ECU flash read/write, use any compatible J2534 flash software, then import the binary into DCTuning Tune Manager.',
  ...
  },
  {
    ...
    vehicles: ['Toyota', 'Subaru', 'Honda', 'Nissan', 'Mitsubishi', 'GM/Opel', 'Ford (legacy)', 'All makes (diagnostics)'],
    software: ['Any J2534 application (ODIS, ISTA, IDS, GDS2, Techstream, Forscan, Clip, Diagbox, BitBox, Combiloader, MDFlasher)'],
    notes: 'Upgraded successor to SM2 Pro — adds CAN FD and DoIP (UDS over Ethernet) support alongside the full SM2 Pro J2534/RP1210 stack. Best budget J2534 adapter for CAN FD vehicles (post-2020). Full J2534-1 and J2534-2 compliant. Works with BitBox, Combiloader, MDFlasher, and all major OEM software portals. USB and Bluetooth versions.',
    appCompat: 'direct-j2534',
    appCompatNote: 'Registers its SM3 J2534 DLL in Windows — auto-detected in DCTuning J2534 PassThru scanner. Full OBD2 diagnostics (DTC, live PIDs, voltage) direct in the app. For ECU flash read/write, use any compatible J2534 flash software, then import the .bin into Tune Manager.',
    recommended: ['Toyota/Subaru OBD flash', 'GM/Opel OBD flash', 'Best budget J2534 hardware', 'CAN FD / post-2020 vehicles'],
  },
  {
    id: 'kt200plus',
    name: 'KT200 Plus / KTflash',
    manufacturer: 'KT200 Plus',
    tier: 'semi-pro',
    priceMin: 120, priceMax: 200,
    iface: ['USB'],
    j2534: true,
    discontinued: false,
    connections: { obd2: true, bench: true },
    caps: { flashRead: true, flashWrite: true, clone: true, eeprom: true, checksum: 'auto', dtc: true, liveData: true, immo: true },
    ecuFamilies: [
      'Bosch EDC16 / ME9 — VAG, BMW (OBD, bench, boot)',
      'Bosch MEDC17 — VAG, BMW, Mercedes, Hyundai/Kia (OBD, bench, boot)',
      'Bosch MD1 / MG1 — VAG MQB/MLB, PSA (OBD, bench)',
      'Siemens/Continental SIMOS 8.x / 12.x / PCR2.x — VAG (OBD, bench)',
      'Siemens/Continental Sxx — Mercedes, Ford SID (OBD)',
      'Siemens/Continental Mxx — BMW MSV80/MSD80/MSD85/MSD87/MSV90 (OBD)',
      'Delphi CRD2.xx / CRD3.xx — various diesel (OBD, bench, boot)',
      'Delphi DCM6.xx — various diesel incl. VAG (OBD, bench)',
      'Delphi DCM7.xx — various diesel (OBD, bench)',
      'Delphi MT series — petrol (OBD, bench)',
      'Delco Exx-Series — GM MPC5777 (E27/E37/E38/E39/E67/E78/E80/E82/E83/E84/E87/E92/E98) (OBD, bench, boot)',
      'Hitachi SH70xx — Nissan (OBD, bench)',
      'Honda Keihin — SH7058, SH72543, TC1782, MPC5566 (OBD, bench)',
      'Denso Toyota GEN1 / GEN2 / GEN4 — Toyota (OBD, bench)',
      'TCU: HITACHI SH72xxx — Mercedes 9G, VW DQ380, AL551, AL450 (bench)',
      'TCU: Delco 6xx — GM 6T30/6T40/6T45/6T50/6T70/6T80/6L45/6L50/6L80/6L90 (bench, boot)',
      'TCU: Infineon — DQ200, DQ250, DL501, DQ382, MPS6, DQ400E (bench)',
      'Ford SIDxx — SID321 (OBD)',
      'Renault/Nissan SIDxx — SID321 (OBD)',
    ],
    vehicles: ['VAG (VW/Audi/Seat/Skoda)', 'BMW', 'Mercedes', 'Toyota', 'Nissan', 'Honda', 'GM/Opel', 'Ford', 'Renault', 'Hyundai/Kia'],
    software: ['KTflash (with dongle for advanced protocols)', 'Any J2534-compliant application'],
    notes: '✅ WORKS WITH DCTUNING: KT200 Plus uses a Scanmatik SM2/SM3 Pro as its J2534 hardware layer — the same adapter we fully support. The KTflash dongle is only needed for KTflash-specific advanced protocols (IMMO off, special clone, TPROT). For DCTuning OBD diagnostics and file-based tuning, no dongle required — install the bundled Scanmatik driver and the device appears automatically in the J2534 PassThru scanner. Supports OBD and bench mode for ECU/TCU read and write. Auto checksum correction for supported ECUs.',
    appCompat: 'direct-j2534',
    appCompatNote: 'Install the bundled Scanmatik driver — the KT200 Plus J2534 interface is auto-detected by DCTuning. Full OBD2 diagnostics (DTC, live PIDs, voltage) work directly. For ECU flash read/write use the included KTflash software or any J2534 tool, then import the .bin into Tune Manager.',
    recommended: ['VAG OBD + bench tuning', 'BMW/Mercedes OBD tuning', 'Multi-make diagnostics', 'TCU tuning (DQ200/DQ380/ZF)'],
  },
  {
    id: 'fordvcm2',
    ...
    ecuFamilies: [
      ...
      'Ford/Mazda Denso — OBD flash',
    ],
    vehicles: ['Ford (all models 1996–present)', 'Mazda (Ford-platform models)', 'Lincoln', 'Mercury'],
    software: ['Ford IDS (reprogramming)', 'Ford FDRS (newer models)', 'ForScan (diagnostics)', 'Any J2534 software'],
    notes: `Ford's OEM diagnostic and reprogramming interface — registers a J2534 DLL in Windows making it compatible with any J2534 flash software.\n\n✅ CLONES: VCM II clones (€25–80 on AliExpress/eBay) are extremely common and most work well with Ford IDS, FDRS, ForScan, and any J2534 flash software for OBD read/write on Ford ECUs. The clone risk is on complex OEM reprogramming sessions — for tuning flash reads, clones are generally reliable. Avoid for warranty/dealer-level work. Look for clones with the MFC chipset for best compatibility.`,
    appCompat: 'direct-j2534',
    appCompatNote: 'Registers a J2534 DLL in Windows — auto-detected in DCTuning J2534 PassThru scanner. App uses it for Ford/Mazda OBD2 diagnostics (DTC, live PIDs). For ECU flash: use any compatible J2534 flash software for Ford T-PROT and Power Stroke modules, then import .bin into Tune Manager.',
  ...
  },
  {
    ...
    ecuFamilies: [
      ...
      'Delphi/Delco E38/E40/E67/E78 — GM (OBD flash)',
      'Denso SH705x — Toyota/Subaru (OBD flash)',
      'Ford T-PROT — Ford/Mazda (via IDS/FDRS)',
    ],
    vehicles: ['All makes with OEM software subscription', 'GM', 'Ford', 'Toyota', 'Chrysler/FCA'],
    software: ['Ford IDS / FDRS', 'GM GDS2 / TIS2Web', 'Chrysler wiTECH', 'Toyota Techstream', 'Any J2534 software'],
    notes: 'Professional J2534 pass-thru from Opus IVS (same maker as CarDAQ-Plus 3). Same vendor as CarDAQ but at a lower price — narrower protocol support (J2534-1 only vs J2534-1+2 on CarDAQ). Widely used by independent shops for Ford, GM, Chrysler OEM reprogramming. Good alternative to CarDAQ for shops that don\'t need the full J2534-2 spec.',
    appCompat: 'direct-j2534',
    appCompatNote: 'Registers Opus IVS J2534 DLL in Windows — auto-detected in DCTuning J2534 PassThru scanner. Full OBD2 diagnostics in the app. For ECU flash read/write, use any compatible J2534 flash software or OEM software, then import .bin into Tune Manager.',
    recommended: ['Ford/GM OBD reprogramming', 'Independent OEM repair shop', 'Multi-make J2534'],
  },
  {
    id: 'dialink',
    name: 'Dialink J2534',
    ...
    ecuFamilies: [
      'Denso SH705x — Toyota, Subaru (OBD flash)',
      'Denso SH72xx — Nissan, Toyota (OBD flash)',
      'Mitsubishi Electric SH7 — OBD flash',
      'Delphi/Delco E38/E40/E67/E78 — GM OBD flash',
      'Continental SIMTEC 75/76 — Opel/Vauxhall OBD flash',
      'Bosch ME7.9.7 — Honda OBD flash',
      'Visteon EEC-V — Ford legacy OBD flash',
      'All OBD2 ECUs — diagnostics',
    ],
    vehicles: ['Toyota', 'Subaru', 'Honda', 'Nissan', 'Mitsubishi', 'GM/Opel', 'Ford (legacy)', 'All makes (diagnostics)'],
    software: ['Any J2534 flash software', 'ForScan', 'Techstream'],
    notes: 'Budget STM32F105-based EU J2534 adapter with excellent OBD flash test results. Full J2534-1 compliant, installs DLL under PassThruSupport.04.04. Very popular in EU tuning community as a low-cost alternative for OBD-only flash operations. OBD only — no bench or boot.',
    appCompat: 'direct-j2534',
    appCompatNote: 'Registers Dialink J2534 DLL in Windows — auto-detected in DCTuning J2534 PassThru scanner. App uses it for full OBD2 diagnostics. For ECU flash, use any compatible J2534 flash software, then import .bin into Tune Manager.',
    recommended: ['Budget J2534 adapter', 'Toyota/Subaru OBD flash', 'EU budget J2534'],
  },
  {
    id: 'godiag',
    ...
    software: ['Any J2534 flash software', 'Honda HDS', 'Toyota Techstream', 'ForScan', 'VAG ODIS (basic)', 'Any ELM327 app (ELM327 mode)'],
    notes: 'Dual-mode device switchable between J2534 PassThru and ELM327 AT-command mode. In J2534 mode it registers a DLL and works with any J2534 software including Techstream, HDS, Forscan, ODIS. In ELM327 mode it acts as a standard OBD2 scanner for Torque, OBD Fusion etc. Budget option — works well for Toyota/Subaru OBD flash. OBD only, no bench/boot. Not recommended for complex OEM reprogramming.',
    appCompat: 'direct-j2534',
    ...
    recommended: ['Budget dual J2534 + ELM327', 'Toyota/Subaru OBD flash on a budget'],
  },
]
          ...
          <div className="grid-2" style={{ gap: 16 }}>
            ...
            <div className="card">
              ...
              {[
                ...
                { q: 'Toyota / Subaru / Honda?', a: 'SH7 BSL bench OR OBD via J2534 flash tool', color: '#f59e0b' },
            ...
            <table className="data-table" style={{ minWidth: 900 }}>
              <thead>
                <tr>
                  ...
                  {['KESS3', 'K-TAG', 'Flex', 'Autotuner', 'CMD', 'BFlash', 'Openport'].map((d) => (
