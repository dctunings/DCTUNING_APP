...
// â”€â”€â”€ Physical-unit safety clamps for signature-driven auto-tuning â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// When we synthesize a MapDef from a scanner match, the catalog gives us a category
// (boost/fuel/torque/â€¦) and a physical unit (hPa, mg/stk, Nm, Â°CRK, â€¦). These clamps
// bound the result of applying a stage multiplier so we never push a map beyond a
// physically safe limit â€” even if the multiplier would allow it.
//
// Values chosen to be ceilings for STREET tuning, not race/motorsport:
//   â€¢ Boost: 3.0 bar absolute (â‰ˆ 2.0 bar of boost above atmospheric) â€” covers
//     Stage 3 tunes on stock TDI/TFSI turbos; race tunes need manual override.
//   â€¢ Fuel: 120 mg/stk for diesel injection quantity â€” Stage 2 territory.
//   â€¢ Torque: 1000 Nm â€” covers nearly everything except V8 TDI / RS-level cars.
//   â€¢ Ignition: 30Â° advance / -10Â° min â€” pushes past any reasonable street knock limit.
//   â€¢ Lambda: 0.75 min / 1.05 max â€” keeps AFR safely rich under load, prevents lean.
//
// Lookups are (category, unit-family) â€” unit strings are normalized (lowercased,
...
export interface PhysicalClamps {
  ...
  min?: number   // physical-unit lower bound (rare â€” mostly lambda/ignition)
}
...
// Boost/pressure unit family â€” covers hPa, mbar, kPa, bar, psi
...
function clampFuel(u: string): PhysicalClamps | undefined {
  ...
  if (u === 'mg/s' || u === 'kg/h')       return { max: 1000 }   // mass air/fuel flow â€” crude ceiling
  if (u === '%')                          return { max: 100 }    // duty / fuel % â€” can't exceed 100
  return undefined
}

function clampTorque(u: string): PhysicalClamps | undefined {
  if (u === 'nm')                         return { max: 1000 }
  if (u === '%')                          return { max: 150 }    // relative torque % â€” allow 50% above stock
  return undefined
}

function clampIgnition(u: string): PhysicalClamps | undefined {
  if (u === 'Â°crk' || u === 'Â°kw' || u === 'deg' || u === 'Â°' ||
      u === 'crkdeg' || u === 'Â°ca')      return { max: 30, min: -10 }
  return undefined
}

function clampEmission(u: string): PhysicalClamps | undefined {
  // Lambda â€” unitless or "-"
  if (u === '' || u === '-' || u === 'lambda' || u === 'l')
                                          return { max: 1.05, min: 0.75 }
  // Temperatures (Â°C, K) â€” don't clamp blindly; target-temp rises are usually fine
  if (u === '%')                          return { max: 100 }    // EGR rate, duty, etc.
  return undefined
}

function clampSmoke(u: string): PhysicalClamps | undefined {
  // Smoke limiters are typically mg/stk or mg â€” same family as fuel
  return clampFuel(u)
}

function clampLimiter(_u: string): PhysicalClamps | undefined {
  // Limiters (RPM, VMAX, etc.) â€” intentionally no clamp. Raising a limiter is the
...
// â”€â”€â”€ Convert physical clamps to raw storage clamps â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
export function physicalToRawClamps(
  ...
  // If factor is negative (rare â€” e.g. inverse scaling), swap max/min so the
  ...
  // Round to integer for int dtypes â€” float32 keeps precision
  if (dtype !== 'float32') {
